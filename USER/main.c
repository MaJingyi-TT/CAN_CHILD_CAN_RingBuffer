...
//			temp_value1 = temp_value1 + ((Current_Value * Volt_Value) / 10000);		//100ms采集一次电功值，并进行累加/*电流乘以电压再乘以时间即为电功值（能量）单位是wh或者kwh*/电功除以标称电压即为电荷值（mah）
//			Battery_Energy = temp_value1;
//			send_pframe.id = ESC2_CONSUMPTION;
//			send_pframe.dlc = 8;
//			send_pframe.data[0] = 0;
//			send_pframe.data[1] = AS_ULONG;
//			send_pframe.data[2] = DUS;
//			send_pframe.data[3] = 0;
//			memcpy(&send_pframe.data[4],&Battery_Energy,4);		
//			CanSendToFifo(&send_pframe);
